(ns careem.main
  "Kotodama WASM entrypoint for the Careem clean-room actor (L4) — Clojure port.
  ...
  facts (`careem.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "careem")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"careem.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "careem-compat" :tier tier :entities entities} 200])
...
(defn start! [] :careem-compat/ready)
